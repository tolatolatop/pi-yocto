...
const evidenceSchema = {
  ...
  properties: {
    ...
    command: { type: "array", items: { type: "string" }, minItems: 1 }, exitCode: { type: "integer" }, jobId: { type: "string" }, workspaceId: { type: "string", pattern: "^[a-f0-9]{64}$" }, provenance: { enum: ["harness-tool", "checkpoint"] }
  },
  allOf: [
...
const phase = { enum: ["INTAKE", "INSPECTING", "PLANNING", "REPLANNING", "WAITING_HUMAN", "EXECUTING", "VERIFYING", "SUMMARIZING", "COMPLETED", "FAILED", "PAUSED"] };
const jobStatus = { enum: ["QUEUED", "RUNNING", "SUCCEEDED", "FAILED", "STOPPING", "STOPPED", "INTERRUPTED"] };
const jobKind = { enum: ["bitbake", "qemu", "check"] };
const jobPurpose = { enum: ["baseline", "diagnostic", "parse", "verification", "incremental-confirmation", "qemu"] };
...
export const schemas: Record<string, unknown> = {
  ...
  JobRecord: {
    ...
    properties: {
      schemaVersion: { const: SCHEMA_VERSION }, id: { type: "string" }, workspaceId: { type: "string", pattern: "^[a-f0-9]{64}$" }, taskId: { type: "string" }, kind: jobKind, purpose: jobPurpose, sourceJobId: { type: "string" }, iteration: { type: "integer", minimum: 1 }, fingerprint: { type: "string" }, executable: { type: "string" }, args: { type: "array", items: { type: "string" } }, cwd: { type: "string" }, status: jobStatus, pid: { type: "integer" }, processGroupId: { type: "integer" }, processStartTicks: { type: "string" }, bootId: { type: "string" }, createdAt: { type: "string" }, startedAt: { type: "string" }, completedAt: { type: "string" }, heartbeatAt: { type: "string" }, exitCode: { type: "integer" }, signal: { type: "string" }, logPath: { type: "string" }, logOffset: { type: "integer", minimum: 0 }, artifacts: { type: "array", items: { type: "string" } }, error: { type: "string" }, resumeCommand: { type: "string" }
    }
  },
  ...
  ChangeSetRecord: {
    ...
    properties: {
      ...
      preflight: { type: "array", items: { type: "object", additionalProperties: false, required: ["kind", "path", "passed", "detail"], properties: { kind: { enum: ["patch-syntax", "patch-applicability", "metadata-review", "semantic-review"] }, path: { type: "string" }, passed: { type: "boolean" }, detail: { type: "string" } } } },
